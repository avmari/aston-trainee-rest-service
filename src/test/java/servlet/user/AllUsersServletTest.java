...
class AllUsersServletTest {
    ...
    private Gson gson;
    @InjectMocks
    private AllUsersServlet allUsersServlet;
    ...
    void doGet() {
        ...
        List<OutgoingUserDto> userDtos = List.of(new OutgoingUserDto(user1.getUsername(), user1.getFirstName(),
                user1.getLastName()),
        ...
        when(gson.toJson(any(List.class))).thenReturn(new Gson().toJson(userDtos));

        ArgumentCaptor<List<OutgoingUserDto>> argumentCaptor = ArgumentCaptor.forClass(List.class);

        allUsersServlet.doGet(httpServletRequest, httpServletResponse);
        ...
        assertEquals(userDtos, argumentCaptor.getValue());
    }
